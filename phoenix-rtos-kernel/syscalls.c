...
int findLongestPath(int pid, int* longestPathLength) {
    int i, pathLength = 0;
    for (i = 0; i < numProcesses; i++) {
        if (processes[i].parentPid == pid) {
            int childPathLength = 0;
            int childPid = processes[i].pid;
            findLongestPath(childPid, &childPathLength);
            if (childPathLength > pathLength)
                pathLength = childPathLength;
        }
    }
    *longestPathLength = pathLength + 1;
    return EOK;
}
...
int syscalls_getLongestPathPid(void* ustack) {
    ...
    for (i = 0; i < 30; i++) {
		...
		// while (currentPid != 0 || pathLength < 5){
		// 	if (i == ignoredPid){
...
int calculatePathLength(int pid) {
    ...
    for (i = 1; i < numProcesses; i++) {
        if (processes[i].parentPid == pid) {
            int childPid = processes[i].pid;
            int childPathLength = calculatePathLength(childPid);
            if (childPathLength > pathLength)
                pathLength = childPathLength;
        }
    }
